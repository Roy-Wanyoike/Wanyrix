import type { StoragePayload, StorageRow } from '@/lib/wanyrix/types'

/**
 * Server-side storage state for `wanyrix storage` (Gate 71.10) — in-process,
 * simulated telemetry. Shared by GET /api/wanyrix/storage and the real
 * reclaim mutation at POST /api/wanyrix/storage/reclaim.
 *
 * Model:
...
function freshRows(): Record<string, RowState> {
  return {
    'Database (SQLite · W-EIR snapshots)': { base: 412, current: 412, reclaimable: false },
...
const KEY = '__wanyrix_storage_state__'
...
export function storagePayload(now = Date.now()): StoragePayload {
  ...
  const rows: StorageRow[] = Object.entries(s.rows).map(([label, r]) => {
    ...
    return {
      ...
      note: r.reclaimable
        ? reclaimedRecently
          ? 'reclaimed — regrows as wanyrix scans and writes (simulated)'
