 ...
 * - Ferrix does NOT know the current contents of the target file, so the
 ...
 *   document inside `ferrix-proposals/`), exactly mirroring what the UI shows:
...
const PROPOSAL_DIR = 'ferrix-proposals'
...
function proposalBody(e: DiffEntry): string[] {
  ...
  if (e.estimate) {
    lines.push(``, `# estimated impact: ${e.estimate} (estimated — verify via ferrix experiment, Gate 21)`)
  }
...
export function buildUnifiedDiff(e: DiffEntry): string {
  const filename = `${PROPOSAL_DIR}/${slugify(e.title || e.target)}${extensionFor(e.kind)}`
 ...
 * Entries are separated by `Index:` blocks; a short comment header explains
 * how to inspect/apply and repeats the Gate-19 notice.
 */
export function buildPatchBundle(entries: DiffEntry[]): string {
  const preamble = [
    `# ferrix proposed-change bundle — ${entries.length} diff${entries.length === 1 ? '' : 's'}`,
    `# generated ${new Date().toISOString()}`,
    `#`,
    `# Each block below is a self-contained unified diff describing a PROPOSAL.`,
    `# Ferrix never modifies a repository silently (Gate 19) — review every`,
    `# block and apply manually if, and only if, the change is wanted.`,
    `# Estimates are estimates: verify via ferrix experiment (Gate 21).`,
    ``,
  ].join('\n')
