 ...
 * its stdout VERBATIM (parsed only to validate the envelope schema). The scan
 * target is the engine crate itself — the tool dogfoods on its own source.
 *
...
/** Repo-root-relative engine crate dir — also the scan target (dogfood). */
...
export async function GET() {
  ...
  try {
    const run = await execFileAsync(
      binary.file,
      ['doctor', '--path', ENGINE_DIR, '--json', '--pretty'],
    ...
    return NextResponse.json(
      {
        error: `engine exited with ${e.code ?? 'non-zero'} — findings are data, but the scan itself failed`,
  ...
  if (!isDoctorEnvelope(report) || report.schema !== 'wanyrix.doctor/v1') {
    return NextResponse.json(
      {
