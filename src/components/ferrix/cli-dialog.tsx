 ...
 * the `ferrix` CLI: same payloads, same exit codes, same `--json` switch on
 * every command. Every command row is copyable (Gate 19 toast on copy).
 */

const COMMANDS: { cmd: string; maps: string }[] = [
  { cmd: 'ferrix doctor', maps: 'Build Doctor view — findings + evidence' },
  { cmd: 'ferrix doctor --json', maps: 'GET /api/ferrix/doctor?ws=…' },
  { cmd: 'ferrix graph --duplicates --json', maps: 'Engineering Graph · duplicates panel' },
  { cmd: 'ferrix impact add-dep <crate> --json', maps: 'Impact Simulator · add a dependency' },
  { cmd: 'ferrix impact upgrade-dep <crate> --json', maps: 'Impact Simulator · upgrade a dependency' },
  { cmd: 'ferrix impact edit-file <path> --json', maps: 'Impact Simulator · edit a source file' },
  { cmd: 'ferrix report --json', maps: 'Topbar Report → JSON snapshot' },
  { cmd: 'ferrix experiment start <finding-id>', maps: 'Experiments view — scaffold from a finding' },
]
