  ...
  const copyComment = async () => {
    try {
      ...
      toast({
        ...
        description: "ferrix[bot]'s regression comment is on your clipboard.",
      })
    } catch {
      toast({
        title: 'Copy failed',
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="flex items-start gap-1.5 text-[11px] leading-snug text-muted-foreground">
        <Info className="mt-0.5 size-3 shrink-0" />
        All impact figures on this page are estimates until verified by a Ferrix experiment (Gate 20).
      </p>
    </div>
  )
}
