...
![Tests](https://img.shields.io/badge/tests-916_passing-2EA043)
...
| | |
| --- | --- |
| 🧪 **916 passing tests** | 920 collected: web 602 in 36 files (0 fail / 4 counted skip) + engine 314 (`cargo test`, 0 fail; 2 opt-in probes ignored) — measured 2026-09-22 same-checkout — unit, live-API contract, conformance, WAL crash-recovery, fault-injection chaos, adversarial hardening, sync roundtrip, offline entitlement, instrumented-build IPC, registration-bridge + local-AI wire-level mock tests |
...
> Counts are measured, not remembered (issue #111), one convention: **N tests, M files, measured 2026-09-22, command**. Command surfaces counted from the `Command` enum in `engine/src/cli.rs` (23); API routes = `find src/app/api -name "route.ts"` (**23, all under `/api/wanyrix/*`** — the scaffold `/api` root was removed in the AUD-6 cleanup); dashboard views = entries in `src/components/wanyrix/nav-registry.ts` (19); web tests = `cd tests && WANYRIX_TEST_BASE_URL=http://localhost:3000 WANYRIX_REQUIRE_LIVE=1 bun test` → **602 tests, 36 files, measured 2026-09-22: 0 fail / 4 counted skip** (the skips are the license-issuance 200-forks — they need `WANYRIX_SIGNING_KEY` on the dev server; the 503-fork test pins the unconfigured behavior). A historical note: runs from a linked git worktree against a dev server rooted at another checkout used to show 3 checkout-root-coupling fails (export-digest path + registration-confinement refusal) — same-root runs are 0-fail and are the reference measurement. Engine tests = `cd engine && cargo test --workspace --offline` → **314 tests, measured 2026-09-22: 314 pass / 0 fail / 2 ignored** (the opt-in `perf_probe` probes).
...
- **916 tests passing, zero product-defect failures** (measured 2026-09-22 same-checkout: engine `cd engine && cargo test --workspace --offline` → 314 pass / 0 fail; web `cd tests && WANYRIX_TEST_BASE_URL=http://localhost:3000 WANYRIX_REQUIRE_LIVE=1 bun test` → 602 run / 0 fail / 4 counted skip) — including live API-contract suites, graph-math invariants (aggregates must equal edge-list closure), honesty-badge WCAG-AA contrast (computed, not asserted), storage-migration goldens, engine conformance + WAL crash-recovery + **9 chaos fault-injection tests** (truncated payloads, garbage DBs, dead sockets, corrupted ledgers) + the 24-test adversarial hardening suite + sync/entitlement CLI tests + wire-level local-AI mock tests.
